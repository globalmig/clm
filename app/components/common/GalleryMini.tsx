      ...
      <Link href={props.link}>
        ...
        <div className="flex justify-between gap-4 flex-wrap mt-4">
          {props.images.map((imgSrc, index) => (
            <div
              key={index}
              className="w-full sm:w-[48%] md:w-[32%] flex flex-col justify-center items-center gap-4 hover:scale-105 transform ease-in-out duration-200 hover:shadow-xl hover:border rounded-xl"
            >
              ...
              <p className="text-center">{props.subcontent}</p>
            </div>
          ))}
        </div>
